...
class Document(HooksMixin, models.Model):
    ...
    * uuid - UUID of a document, universally Unique ID. An unique identifier
    ...
    def get_request_url(self):
        return reverse('documents:document_request', args=(self.pk,))
    ...
    def get_truncated_label(self):
        label = os.path.splitext(self.label)[0]
        if len(label) > 22:
            label = self.label[:22] + '...'
            return label.title()
        else:
    ...
    def request_permission(self):
        user_list = []
        acls = AccessControlList.objects.filter(object_id = self.id)
        for acl in acls:
            if 'Edit ACLs' in acl.get_permission_titles():
                groups = acl.role.groups
...
class DocumentVersion(models.Model):
    ...
    * checksum - A hash/checkdigit/fingerprint generated from the document's
    ...
    def save(self, *args, **kwargs):
        ...
        """
        user = kwargs.pop('_user', None)
        is_stub = kwargs.pop('is_stub',False)

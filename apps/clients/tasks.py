...
def create_report1(queryset,title,total, period,today,register=False, user=None):
    ...
    for r in queryset:
        ...
        if register:
            pdf.cell(pdf.column_group_width, pdf.row_height, r.group.name, pdf.row_border)
...
class ClientListPDF(FPDF):
    ...
    row_border = 0
    row_header_border = 0
    def __init__(self,title, subtitle, total):
        self.image_path = staticfiles_storage.path("appearance/images/docs96x96.png")
        self.title = title
    ...
    def header(self):
        ...
        if self.total:
            self.cell(20, 4, 'Total:', 0, 0, 'L')
            self.cell(0, 4, str(self.total), 0, 1, 'L')
        self.cell(0, 4, self.subtitle, 0, 1, 'L')
        self.cell(0, 4, self.today, 0, 1, 'L')
        y = self.get_y()
        x = self.get_x()
        self.line(x,y+5,200,y+5)
...
def create_report(data,title,total):
    pdf = ClientListPDF(title=title, subtitle=data[8], total = total)
    pdf.add_page()
    pdf.set_font('Arial', 'B', 10)
    pdf.set_auto_page_break(False)
    pdf.cell(pdf.column_client_width, pdf.row_header_height, 'Name',pdf.row_header_border)
    ...
    pdf.cell(pdf.column_misc_width, pdf.row_header_height, 'Misc.', pdf.row_header_border)
    pdf.cell(pdf.column_total_width, pdf.row_header_height, 'Total', pdf.row_header_border,1)

    pdf.set_font('Arial','',10)
    for d in data[0]:

        pdf.cell(pdf.column_client_width, pdf.row_height, d.name,pdf.row_border)
        if data[1]:
            pdf.cell(pdf.column_active_width, pdf.row_height, str(d.get_no_matters('Active',data[1])),pdf.row_border)
            pdf.cell(pdf.column_not_active_width, pdf.row_height, str(d.get_no_matters('Not active',data[1])),pdf.row_border)
            pdf.cell(pdf.column_dormant_width, pdf.row_height, str(d.get_no_matters('Dormant',data[1])),pdf.row_border)
            pdf.cell(pdf.column_closed_width, pdf.row_height, str(d.get_no_matters('Closed',data[1])),pdf.row_border)
            pdf.cell(pdf.column_misc_width, pdf.row_height, str(d.get_no_matters('Misc',data[1])),pdf.row_border)
            pdf.cell(pdf.column_total_width, pdf.row_height, str(d.get_no_matters(lawyers=data[1])),pdf.row_border,1)
        else:
            pdf.cell(pdf.column_active_width, pdf.row_height, str(d.get_no_matters('Active')),pdf.row_border)
            pdf.cell(pdf.column_not_active_width, pdf.row_height, str(d.get_no_matters('Not active')),pdf.row_border)
            pdf.cell(pdf.column_dormant_width, pdf.row_height, str(d.get_no_matters('Dormant')),pdf.row_border)
            pdf.cell(pdf.column_closed_width, pdf.row_height, str(d.get_no_matters('Closed')),pdf.row_border)
            pdf.cell(pdf.column_misc_width, pdf.row_height, str(d.get_no_matters('Misc')),pdf.row_border)
            pdf.cell(pdf.column_total_width, pdf.row_height, str(d.get_no_matters()),pdf.row_border,1)
        y = pdf.get_y()
        if y > 270.0:
            pdf.add_page()
    pdf.set_font('Arial','B',10)
    pdf.cell(pdf.column_client_width, pdf.row_height, '',pdf.row_border)
    pdf.cell(pdf.column_active_width, pdf.row_height, str(data[2]),pdf.row_border)
    pdf.cell(pdf.column_not_active_width, pdf.row_height, str(data[3]),pdf.row_border)
    pdf.cell(pdf.column_dormant_width, pdf.row_height, str(data[4]),pdf.row_border)
    pdf.cell(pdf.column_closed_width, pdf.row_height, str(data[5]),pdf.row_border)
    pdf.cell(pdf.column_misc_width, pdf.row_height, str(data[6]),pdf.row_border)
    pdf.cell(pdf.column_total_width, pdf.row_height, str(data[7]),pdf.row_border,1)

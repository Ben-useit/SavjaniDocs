...
from .forms import IndexListForm, IndexTemplateNodeForm, IndexSearchForm
    ...
    def get_object_list(self):
        if self.index_instance_node:
            if self.index_instance_node.index_template_node.link_documents:
                ...
                self.form = IndexSearchForm( node = self.index_instance_node)
...
def index_node_search(request):

    if request.method == "POST":
        target = '/indexing/instance/node/' + request.POST.get('search_input')

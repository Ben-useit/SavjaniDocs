...
class IndexSearchForm(forms.Form):
    search_input = forms.ModelChoiceField(
        label=_(''), queryset=None,
        widget=forms.Select(
            attrs={'class': 'select2','onChange':'form.submit();'},
        )
    )
    ...
    def __init__(self, node=None, *args, **kwargs):
        if node:
            queryset = kwargs.pop(
                'queryset', node.get_children().order_by('value')
            )
        else:
        ...
        self.fields['search_input'].queryset = queryset   

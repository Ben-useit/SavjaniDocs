...
from ..tasks import task_delete_document, task_update_page_count, task_send_mail
...
class DocumentListView(SingleObjectListView):
    ...
    def get_document_queryset(self):
        return Document.objects.defer(
            ...
            'deleted_date_time'
        ).exclude(register__status__name="Closed")
...
def document_request(request, pk):
    doc = Document.objects.get(pk =pk)
    users = doc.request_permission()
    con = {'request_document' : True, 'doc' : doc , 'title' : 'You need permission to view this document' ,
        'users':users,  }

    if request.method == 'POST':
        task_send_mail(document = doc,permission_holders = users,user=request.user)
        messages.success(
            request, _('Permission request for document %(document) sent successfully.') % {
                'document': doc
            }
        )


    return render(request, 'appearance/generic_form.html',context= con)

def permission_request_confirmation(request):
    pass
...
class RecentAddedDocumentListView(DocumentListView):
    ...
    def get_document_queryset(self):
        ...
        user=self.request.user
        actions = Action.objects.filter(verb="documents.document_new_version").filter(actor_object_id=user.id).order_by('-timestamp')[:30]
        my_doc_ids = []
        for a in actions:
            my_doc_ids.append(a.action_object.id)
        return Document.objects.filter(pk__in = my_doc_ids).order_by('-date_added')
        ...
        context.update(
            {
                ...
                'slice_count': setting_recent_added_count.value,
            }
        )
        return context
